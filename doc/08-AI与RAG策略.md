...
1. 文档解析/切分/向量化必须以异步任务运行，任务状态写入 `job_runs`。
2. 任一步骤失败必须记录可读错误，并按重试策略执行（指数退避，最大重试次数可配置）。
3. 任务幂等：同一 `document_id + content_hash` 不得重复入库。
4. 内部服务请求与事件载荷格式见 `12-内部服务契约与异步任务规范.md`。
...
1. AI 出题场景必须通过 JSON Schema 校验后才能入库；校验失败触发一次“修复性重试”。
2. 教案生成返回必须满足固定章节结构（见 5.4），章节缺失视为无效输出。
3. 聊天场景若返回引用，引用字段必须与检索结果 `document_id/chunk_index` 对齐。

## 12. 评测与回归要求

1. 维护最小离线评测集（聊天 50 条、出题 50 条、解析 50 条、教案 20 条）。
2. 每次模型路由或 Prompt 版本变更必须执行离线评测，结果写入评测报告。
3. 回归门禁：
   - 聊天引用率下降不得超过 3 个百分点。
   - AI 出题 JSON 合法率不得低于 95%。
   - 空命中场景幻觉率必须为 0。

---
文档状态：`v1.2.0`（2026-02-26 新增异步编排、结构化校验与评测回归要求）

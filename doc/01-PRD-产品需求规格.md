...
## 7. 验收定义（全局）
仅当以下条件全部满足时，项目可判定“需求完成”：

1. `02-需求追踪矩阵.md` 中所有 in-scope 需求状态为完成。
2. `06-API契约-openapi.yaml` 所有 in-scope 接口可被调用。
3. `05-数据模型与迁移规范.md` 结构已落库并可支撑业务。
4. `11-验收清单与演示脚本.md` 核心场景全部通过。

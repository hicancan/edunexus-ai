...
| R-PLT-05 | API 与 AI 服务内部契约标准化 | `12-内部服务契约` 定义的接口、幂等键、超时与重试策略全部落地 |
| R-PLT-06 | 建立测试金字塔与质量门禁 | 单元/集成/E2E/契约测试可执行，门禁阈值满足 `13-测试策略` |
| R-PLT-07 | CI/CD 与环境一致性保障 | PR 必跑 lint/type/test/security；主干可自动构建部署 |
| R-PLT-08 | 安全基线内建 | SAST/SCA/Secret Scan 与依赖漏洞门禁默认开启 |
| R-PLT-09 | 前端实现规范可执行 | 页面实现满足 `03 + 14` 的交互、响应式、可访问性约束 |
| R-PLT-10 | Python 运行环境强约束 | AI 服务与脚本必须在 Conda `edunexus-ai` 环境并使用 `uv` 执行 |
...
5. 核心 API（P95）目标：登录 <= 800ms，题目提交 <= 1200ms，聊天首包 <= 3000ms。
6. 可用性目标：核心链路月度可用性 >= 99.5%。
7. 可维护性目标：关键模块单测覆盖率 >= 80%，核心领域服务 >= 90%。
8. 安全目标：高危漏洞（CVSS >= 7.0）不得带入发布分支。
9. Python 运行环境必须是 Conda `edunexus-ai` 且使用 `uv`，不接受其它环境名或执行方式。

## 7. 验收定义（全局）

完整 DoD 与验收用例见 `11-验收清单与演示脚本.md`，质量门禁见 `13-测试策略与质量门禁.md`。

---
文档状态：`v1.2.0`（2026-02-26 新增 R-PLT-05~10，补充可观测/性能/安全/环境隔离非功能要求）

...
7. 教师访问学生学情：除角色为 TEACHER 外，还必须满足 `teacher_student_bindings(teacher_id, student_id, status=ACTIVE)`。

任何归属校验失败返回 403。

## 3.3 服务间授权（M2M）

1. API 服务调用 AI 服务必须携带服务凭证（Service Token 或 mTLS 身份）。
2. AI 服务仅接受来自受信 API/Worker 的调用，拒绝匿名公网直连。
3. 服务凭证泄露视为 P0 安全事件，必须即时轮换。
...
补充约束：教师端关于学生数据的接口（analytics/suggestions）在角色通过后仍需做教学绑定校验。
...
5. 管理员下载操作必须有审计记录。
6. 服务间调用密钥不得写入代码库，必须通过环境变量或密钥管理系统注入。
7. 分享链接访问应支持时效或撤销策略，撤销后访问返回 404。

---
文档状态：`v1.2.0`（2026-02-26 新增教学绑定校验与服务间授权规则）

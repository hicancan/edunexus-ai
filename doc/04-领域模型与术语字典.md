...
## 2.9 TeachingRoster（教学授权关系域）

- 核心实体：`Classroom`、`TeacherStudentBinding`
- 职责：定义教师可查看/可干预的学生集合
- 关键不变量：
  1. 教师访问学生学情必须存在有效绑定关系。
  2. 绑定关系必须可审计、可撤销。
  3. 同一教师-学生在同一作用域下只能存在一条 ACTIVE 绑定。
...
1. `PENDING`：已创建待生效。
2. `ACTIVE`：教师可访问该学生授权范围数据。
3. `REVOKED`：授权撤销，教师后续不可访问。
...
| 教学绑定 | Teaching Binding | 教师与学生之间的数据访问授权关系 |
...
7. 教学绑定状态：`PENDING`、`ACTIVE`、`REVOKED`
8. 班级状态：`ACTIVE`、`ARCHIVED`
9. 任务状态：`PENDING`、`RUNNING`、`SUCCEEDED`、`FAILED`、`DEAD_LETTER`
...
5. 教师对学生数据的访问不仅受角色控制，还受 TeachingRoster 授权关系控制（RBAC + ABAC）。

---
文档状态：`v1.2.0`（2026-02-26 新增 TeachingRoster 域与教学授权状态机）

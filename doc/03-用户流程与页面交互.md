...
补充说明：组件级实现细则与视觉/可访问性约束见 `14-前端实现规范与设计系统约束.md`。
...
6. 所有写接口必须具备防重复提交策略（前端按钮防抖 + 后端幂等键或去重策略）。
7. 列表与表单交互必须支持键盘导航，关键按钮具备可访问名称（a11y）。
8. 断点规范：`>=1280` 桌面、`768~1279` 平板、`<=767` 手机；核心链路在三类断点均可用。

## 3.1 路由鉴权与可见性约束

1. 未登录用户仅可访问 `/login`、`/register`、`/404`。
2. STUDENT 不可访问 `/teacher/*`、`/admin/*`；TEACHER 不可访问 `/student/*`、`/admin/*`。
3. ADMIN 只访问 `/admin/*` 与公共路由，不进入学生/教师业务流页面。
4. 命中无权限路由必须重定向到 `/403`，且保留来源路径用于审计。
...
5. 页面在移动端（<=767）不出现核心操作不可达（按钮被遮挡/表单不可提交）。
6. 富文本内容（AI 回答/教案 Markdown）渲染前必须经过 XSS 白名单净化。
7. 所有异步列表默认保留上一次成功数据并展示刷新中状态，避免白屏抖动。

---
文档状态：`v1.1.0`（2026-02-26 增补路由鉴权、响应式与可访问性约束，并引用 14）

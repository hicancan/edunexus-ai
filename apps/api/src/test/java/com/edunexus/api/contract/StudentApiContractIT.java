...
class StudentApiContractIT extends ApiContractIntegrationBase {
    ...
    void chatEndpoints_shouldMatchContract() throws Exception {
        ...
        String createBody =
                mockMvc.perform(
                                post("/api/v1/student/chat/session")
        ...
        mockMvc.perform(
                        get("/api/v1/student/chat/sessions")
                                .param("page", "1")
                                .param("size", "20")
                                .header("Authorization", "Bearer " + token))
        ...
        mockMvc.perform(
                        post("/api/v1/student/chat/session/{sessionId}/message", sessionId)
                                .header("Authorization", "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        objectMapper.writeValueAsString(
                                                Map.of("message", "请解释牛顿第二定律"))))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(
                        get("/api/v1/student/chat/session/{sessionId}", sessionId)
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(
                        delete("/api/v1/student/chat/session/{sessionId}", sessionId)
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
    ...
    void exerciseEndpoints_shouldMatchContract() throws Exception {
        ...
        String questionsBody =
                mockMvc.perform(
                        ...
                        .andExpect(jsonPath("$.data.content", notNullValue()))
                        .andExpect(jsonPath("$.data.page").value(1))
                        .andExpect(jsonPath("$.data.content[0].correctAnswer").doesNotExist())
                        .andReturn()
                        .getResponse()
                        .getContentAsString();
        ...
        mockMvc.perform(
                        get("/api/v1/student/exercise/{recordId}/analysis", recordId)
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
        ...
        String wrongBody =
                mockMvc.perform(
                                get("/api/v1/student/exercise/wrong-questions")
                                        .param("status", "ACTIVE")
                                        .header("Authorization", "Bearer " + token))
                        .andExpect(status().isOk())
                        .andExpect(jsonPath("$.data.content", notNullValue()))
        ...
        mockMvc.perform(
                        delete(
                                        "/api/v1/student/exercise/wrong-questions/{questionId}",
                                        wrongQuestionId)
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").isEmpty());
        ...
        mockMvc.perform(
                        get("/api/v1/student/exercise/records")
                                .param("page", "1")
                                .param("size", "20")
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
    ...
    void aiQuestionEndpoints_shouldMatchContract() throws Exception {
        ...
        String generateBody =
                mockMvc.perform(
                                post("/api/v1/student/ai-questions/generate")
                                        .header("Authorization", "Bearer " + token)
        ...
        mockMvc.perform(
                        get("/api/v1/student/ai-questions")
                                .param("page", "1")
                                .param("size", "20")
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
        ...
        String submitBody =
                mockMvc.perform(
                                post("/api/v1/student/ai-questions/submit")
                                        .header("Authorization", "Bearer " + token)
        ...
        String recordId = objectMapper.readTree(submitBody).path("data").path("recordId").asText();
        mockMvc.perform(
                        get("/api/v1/student/ai-questions/{recordId}/analysis", recordId)
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.recordId").value(recordId))
                .andExpect(jsonPath("$.data.items", notNullValue()));

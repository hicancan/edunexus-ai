...
public class AuthController implements ControllerSupport {
    ...
    public ResponseEntity<ApiResponse> register(
            @Valid @RequestBody RegisterReq req, HttpServletRequest request) {
        if (db.exists(
                "select 1 from users where username=? and deleted_at is null", req.username())) {
    ...
    public ResponseEntity<ApiResponse> login(
            @Valid @RequestBody LoginReq req, HttpServletRequest request) {
    ...
    public ResponseEntity<ApiResponse> refresh(
            @Valid @RequestBody RefreshReq req, HttpServletRequest request) {
        Claims claims;
        try {
        ...
        Map<String, Object> user =
                db.oneOrNull("select * from users where id=? and deleted_at is null", userId);
        if (user == null) {
            throw new UnauthorizedException(ErrorCode.AUTH_TOKEN_INVALID, "refresh token 无效");
        }
    ...
    public ResponseEntity<ApiResponse> me(HttpServletRequest request) {
        ...
        Map<String, Object> user =
                db.one("select * from users where id=? and deleted_at is null", authUser.userId());
        return ResponseEntity.ok(ApiResponse.ok(toUserVo(user), trace(request)));
    }

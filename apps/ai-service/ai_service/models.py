...
def _normalize_choice_answer(value: Any) -> str:
    ...
    if not answer:
        return ""
    letters = sorted({char for char in answer if "A" <= char <= "Z"})
    return "".join(letters)
...
class ChatRequest(BaseModel):
    ...
    teacher_scope: TeacherScope | None = Field(
        default=None, validation_alias=AliasChoices("teacherScope", "teacher_scope")
    )
    teacher_id: str | None = Field(
        default=None, validation_alias=AliasChoices("teacherId", "teacher_id")
    )
...
class WrongAnalyzeRequest(BaseModel):
    ...
    user_answer: str = Field(
        validation_alias=AliasChoices(
            "userAnswer", "user_answer", "studentAnswer", "student_answer"
        )
    )
    ...
    knowledge_points: list[str] = Field(
        ...
        validation_alias=AliasChoices(
            "knowledgePoints", "knowledge_points", "conceptTags", "concept_tags"
        ),
    )
    teacher_suggestion: str | None = Field(
        default=None,
        validation_alias=AliasChoices("teacherSuggestion", "teacher_suggestion"),
    )
...
class AIQGenerateRequest(BaseModel):
    ...
    student_id: str | None = Field(
        default=None, validation_alias=AliasChoices("studentId", "student_id")
    )
    ...
    concept_tags: list[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("conceptTags", "concept_tags"),
    )
...
class LessonPlanRequest(BaseModel):
    ...
    duration_mins: int = Field(
        validation_alias=AliasChoices("durationMins", "duration_mins"), ge=10, le=240
    )
    teacher_id: str | None = Field(
        default=None, validation_alias=AliasChoices("teacherId", "teacher_id")
    )
...
class KbIngestRequest(BaseModel):
    ...
    job_id: str | None = Field(
        default=None, validation_alias=AliasChoices("jobId", "job_id", "run_id")
    )
    document_id: str
    teacher_id: str
    class_id: str
...
class AIQuestionItem(BaseModel):
    ...
    question_type: str = Field(
        ...
        validation_alias=AliasChoices("question_type", "questionType", "type"),
    )
    content: str
    options: dict[str, str] = Field(default_factory=dict)
    correct_answer: str = Field(
        validation_alias=AliasChoices("correct_answer", "correctAnswer", "answer")
    )
    explanation: str
    knowledge_points: list[str] = Field(
    ...
    @classmethod
    def _normalize_question_type(cls, value: Any) -> str:
        question_type = _clean_text(value).upper()
        if not question_type:
            question_type = "SINGLE_CHOICE"
        # Remap legacy/variant types to the canonical set Java accepts
        remap = {
        ...
        question_type = remap.get(question_type, question_type)
        allowed = {"SINGLE_CHOICE", "MULTIPLE_CHOICE", "SHORT_ANSWER"}
        if question_type not in allowed:
            raise ValueError(f"question_type '{question_type}' is unsupported")
    ...
    @classmethod
    def _validate_options(cls, value: dict[str, str], info: Any) -> dict[str, str]:
        question_type = str((info.data or {}).get("question_type", "SINGLE_CHOICE"))
    ...
    def _validate_answer_in_options(cls, value: str, info: Any) -> str:
        ...
        if question_type == "MULTIPLE_CHOICE":
            normalized = _normalize_choice_answer(value)
            if len(normalized) < 2:
                raise ValueError("MULTIPLE_CHOICE correct_answer must contain 2+ options")

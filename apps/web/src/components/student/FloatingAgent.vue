...
const currentMessage = ref({
  title: "学习提醒",
  content: "检测到近期答题中存在高频错误，建议立即进入对话进行针对性复盘。"
});
...
onMounted(async () => {
  try {
    const weakPoints = await listProfileWeakPoints();
    ...
    currentMessage.value = {
      title: "发现薄弱点",
      content: `「${top.knowledgePoint || "未命名知识点"}」近期失误率 ${Number(top.errorRate || 0).toFixed(1)}%，建议立即复盘。`
    };
      ...
      <div v-if="isExpanded" class="agent-panel glass-card">
        <div class="panel-header">
          <div class="header-title">
            ...
            <span>EduNexus 学习助手</span>
          </div>
          <button class="close-btn" aria-label="关闭" @click="dismissAgent">
            <X :size="16" />
          </button>
        </div>

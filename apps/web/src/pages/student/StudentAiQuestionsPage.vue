...
const conceptTagsInputProps = {
  id: "aiq-concept-tags",
  name: "aiqConceptTags",
  "aria-label": "AI 出题知识点"
};
const historySubjectInputProps = {
  id: "aiq-history-subject",
  name: "aiqHistorySubject",
  "aria-label": "AI 出题历史筛选学科"
};
...
function isShortAnswer(questionType?: string): boolean {
...
function parseMultipleChoiceAnswer(raw: string): string[] {
  return Array.from(
    new Set(
      (raw || "")
        .toUpperCase()
...
function updateSingleAnswer(questionId: string, value: string): void {
  answerMap.value[questionId] = value;
}

function updateMultipleAnswer(questionId: string, values: Array<string | number>): void {
  answerMap.value[questionId] = Array.from(
...
async function generateQuestions(): Promise<void> {
  ...
  // generate() handles errors internally and sets aiqStore.error (shown via alert)
  if (aiqStore.error) return;

  if (aiqStore.generatedQuestions.length === 0) {
    message.warning("AI 未能生成题目，请稍后重试");
    return;
  }

  answerMap.value = {};
  message.success(`成功生成 ${aiqStore.generatedQuestions.length} 道题目`);
  await loadHistory();
}
...
const historyColumns: DataTableColumns<any> = [
  ...
  },
  {
    ...
    render(row) {
      ...
      return h(NSpace, { align: "center", justify: "center", size: 8 }, () => [
        h(
          NText,
          { type: "success", strong: true },
          { default: () => `${Number(row.correctRate || 0).toFixed(0)}%` }
        ),
  ...
  },
  {
    ...
    render(row) {
      ...
      return h(
        NButton,
        { size: "small", type: "primary", secondary: true, onClick: () => viewAnalysis(recordId) },
    ...
    <n-space vertical :size="24">
      ...
      <n-card :bordered="true" title="配置出题要求" size="small">
        ...
        <n-form inline :model="form" label-placement="left" :show-feedback="false">
          <n-form-item label="学科" :label-props="{ for: 'aiq-subject' }">
            <n-input
          ...
          <n-form-item label="题目数量" :label-props="{ for: 'aiq-count' }">
            <n-input-number
          ...
          <n-form-item label="知识点" :label-props="{ for: 'aiq-concept-tags' }">
            <n-input
              v-model:value="form.conceptTagsText"
              placeholder="逗号分隔，可选"
              :input-props="conceptTagsInputProps"
            />
          </n-form-item>
        </n-form>
      </n-card>
      ...
      <n-spin :show="aiqStore.generateLoading">
        <n-empty
          v-if="aiqStore.generatedQuestions.length === 0"
          description="尚未生成题目"
          style="margin: 40px 0"
        />
        ...
        <div v-else class="question-list">
          ...
          >
            ...
            <div class="answer-area">
              <n-checkbox-group
                v-if="isMultipleChoice(question.questionType)"
                :value="parseMultipleChoiceAnswer(answerMap[question.id as string] || '')"
                @update:value="(values) => updateMultipleAnswer(question.id as string, values)"
              >
                <n-space vertical :size="12">
                  <n-checkbox
                    ...
                    :value="key"
                  >
                    {{ key }}. {{ formatQuestionOption(value) }}
                  </n-checkbox>
                </n-space>
              </n-checkbox-group>

              <n-radio-group
                v-else-if="!isShortAnswer(question.questionType)"
                :value="answerMap[question.id as string]"
                :name="`answer-${question.id}`"
                @update:value="(value) => updateSingleAnswer(question.id as string, value)"
              >
                <n-space vertical :size="12">
      ...
      >
        ...
        <n-space vertical :size="12">
          <div
            v-for="(item, index) in aiqStore.submitResult.items || []"
            :key="item.questionId"
          ...
          >
            ...
            <n-space :size="20">
              <n-text depth="2"
                >你的答案：<n-text strong :type="item.isCorrect ? 'success' : 'error'">{{
                  item.userAnswer
                }}</n-text></n-text
              >
              <n-text depth="2"
                >正确答案：<n-text strong type="success">{{ item.correctAnswer }}</n-text></n-text
              >
              <n-text depth="2"
                >得分：<n-text strong>{{ item.score }}</n-text></n-text
              >
            </n-space>
          </div>
        </n-space>
      ...
      >
        <n-space vertical :size="16">
          <div
            v-for="item in currentAnalysis.items || []"
            :key="item.questionId"
            class="analysis-item-box"
          >
            <n-text strong class="analysis-title">题目: {{ item.content }}</n-text>
            <div class="analysis-badge-row">
              <n-tag :type="item.isCorrect ? 'success' : 'error'" size="small">{{
                item.isCorrect ? "正确" : "错误"
              }}</n-tag>
              <n-text depth="3"
                >| 你的答案：{{ item.userAnswer }} | 正确答案：{{ item.correctAnswer }}</n-text
              >
            </div>
      ...
      <!-- History Section -->
      <n-card :bordered="true" title="生成历史" size="small" class="history-card">
        <template #header-extra>
          <n-form
            inline
            :model="historyFilter"
            ...
            size="small"
          >
            <n-form-item label="学科" :label-props="{ for: 'aiq-history-subject' }">
              <n-input
                v-model:value="historyFilter.subject"
                placeholder="过滤学科"
                clearable
                style="width: 140px"
                :input-props="historySubjectInputProps"
                @keydown.enter="applyHistoryFilters"
              />
            </n-form-item>

...
function getActionColor(action: string): "default" | "error" | "info" | "success" | "warning" {
  ...
  if (lowered.includes("create") || lowered.includes("import") || lowered.includes("generate"))
    return "success";
  if (lowered.includes("delete") || lowered.includes("remove") || lowered.includes("destroy"))
    return "error";
  if (lowered.includes("update") || lowered.includes("edit") || lowered.includes("modify"))
    return "warning";
  if (lowered.includes("login") || lowered.includes("auth") || lowered.includes("read"))
    return "info";
  return "default";
}
    ...
    render(row) {
      if (!row.actorId) {
        return h(NSpace, { align: "center", size: 4 }, () => [
          ...
          h(NText, { type: "info", strong: true }, { default: () => "系统任务" })
        ]);
      }
      return h(NSpace, { vertical: true, size: 2 }, () => [
        ...
        h(
          ...
          { default: () => `角色: ${row.actorRole || "GUEST"}` }
        )
      ]);
    }
  },
  ...
  <div class="audits-page">
    <n-space vertical :size="16">
      <div class="page-header">
        <div>
          <n-text tag="h2" class="page-title">审计日志</n-text>
          <n-space align="center" :size="6" style="margin-top: 4px">
            <ShieldAlert :size="14" style="color: #64748b" />
            <n-text depth="3">展示平台关键操作记录，用于追踪用户、资源和系统行为。</n-text>
          </n-space>
        </div>
        <n-button
          type="primary"
      ...
      <n-alert v-if="adminStore.auditsError" type="error" :show-icon="true">{{
        adminStore.auditsError
      }}</n-alert>

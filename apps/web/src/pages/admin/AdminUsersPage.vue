...
async function saveUser(userId: string): Promise<void> {
  ...
  try {
    ...
    if (updated) {
      message.success(`用户 ${updated.username} 已更新`);
      await loadUsers();
    }
  } finally {
    edit.saving = false;
  ...
  },
  {
    ...
    render(row) {
      ...
      return h(
        ...
        {
          default: () => [
            h(NRadioButton, { value: "STUDENT", label: "学生" }, { default: () => "学生" }),
            h(NRadioButton, { value: "TEACHER", label: "教师" }, { default: () => "教师" }),
            h(NRadioButton, { value: "ADMIN", label: "管理员" }, { default: () => "管理员" })
          ]
        }
      );
    }
  },
  {
    ...
    render(row) {
      ...
      return h(
        ...
        {
          default: () => [
            h(NRadioButton, { value: "ACTIVE", label: "启用" }, { default: () => "启用" }),
            h(NRadioButton, { value: "DISABLED", label: "停用" }, { default: () => "停用" })
          ]
        }
      );
    }
  },
  {
    ...
    render(row) {
      ...
      return h(
        ...
        { default: () => "保存", icon: () => h(Save, { size: 14 }) }
      );
    }
  },
  {
  ...
  <div class="admin-users-page">
    <n-space vertical :size="16">
      <div class="page-header">
        <div>
          <n-text tag="h2" class="page-title">用户管理</n-text>
          <n-text depth="3">负责账号创建、角色调整和状态管理。</n-text>
        </div>
      </div>
        ...
        <n-space justify="space-between" align="center">
          <n-form inline :model="filters" label-placement="left" :show-feedback="false">
            <n-form-item label="角色">
              <n-select v-model:value="filters.role" :options="roleOptions" style="width: 140px" />
            </n-form-item>
          ...
          <n-button type="primary" class="animate-pop" @click="showCreateModal = true">
      ...
      <n-alert v-if="adminStore.usersError" type="error" :show-icon="true">{{
    ...
    >
      ...
      >
        <n-form-item label="用户名" required>
          <n-input v-model:value="createForm.username" placeholder="3-50 位字母、数字或下划线" />
        </n-form-item>
        ...
        <n-form-item label="手机号">
          <n-input v-model:value="createForm.phone" placeholder="手机号（可选）" />
        </n-form-item>
      </n-form>
      <template #action>
        <n-space justify="end">
          <n-button class="animate-pop" @click="showCreateModal = false">取消</n-button>
          <n-button
            type="primary"
            :loading="adminStore.operationLoading"
            class="animate-pop"
            @click="createUser"
            >创建</n-button
          >

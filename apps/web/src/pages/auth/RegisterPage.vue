      ...
      <n-card class="register-card glass-card" :bordered="false" size="large">
        <template #header>
          <div class="header-titles">
            <n-text tag="h1" class="main-title">账号注册</n-text>
            <n-text depth="3" class="sub-title"
              >支持学生与教师自助注册，管理员账号由平台统一创建。</n-text
            >
          </div>
        </template>
        ...
        <n-space vertical :size="20">
          ...
          <n-form @submit.prevent="submitRegister">
            <n-form-item label="用户名" path="username" :label-props="{ for: 'register-username' }">
              <n-input
            ...
            <div class="role-group" role="group" aria-labelledby="register-role-label">
              <span id="register-role-label" class="role-group__label">身份类型</span>
              <fieldset class="role-fieldset">
                <legend class="sr-only">身份类型</legend>
            ...
            <n-form-item label="密码" path="password" :label-props="{ for: 'register-password' }">
              <n-input
            ...
            <n-form-item
              label="手机号（可选）"
              path="phone"
              :label-props="{ for: 'register-phone' }"
            >
              <n-input
                v-model:value="phone"
                :input-props="{
            ...
            <n-form-item label="邮箱（可选）" path="email" :label-props="{ for: 'register-email' }">
              <n-input
                v-model:value="email"
                :input-props="{
                  id: 'register-email',
            ...
            <n-space justify="end" class="form-actions" :size="16">
              <n-button text class="animate-pop" @click="router.push('/login')">
                返回登录
              </n-button>
              <n-button
                type="primary"

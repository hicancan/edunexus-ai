...
const timeoutFromEnv = Number(import.meta.env.VITE_API_TIMEOUT_MS || 60000);
const DEFAULT_TIMEOUT_MS =
  Number.isFinite(timeoutFromEnv) && timeoutFromEnv > 0 ? timeoutFromEnv : 60000;
...
export class ApiClientError extends Error {
  ...
  constructor(
    message: string,
    options?: { code?: string | number; status?: number; traceId?: string }
  ) {
    super(message);
...
export function getAccessTokenForRequest(): string {
...
function readRefreshToken(): string {
  ...
  return (
    ...
    localStorage.getItem(legacyRefreshTokenKey) ||
    ""
  );
}
...
apiClient.interceptors.request.use((config) => {
  ...
  if (
    ...
    !hasHeader(requestConfig, "Idempotency-Key")
  ) {
...
apiClient.interceptors.response.use(
  (response) => {
    ...
    const traceId =
      (response.data as ApiEnvelope | undefined)?.traceId || response.headers["x-trace-id"] || "";

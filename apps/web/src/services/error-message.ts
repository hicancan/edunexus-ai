...
const errorMessageMap: Record<string, string> = {
  ...
  AI_TIMEOUT: "AI 生成超时，请稍后重试或降低任务复杂度",
...
export function toErrorMessage(error: unknown, fallback = "操作失败，请稍后重试"): string {
  if (error instanceof ApiClientError) {
    if (/DEADLINE_EXCEEDED|timeout/i.test(error.message || "")) {
      return errorMessageMap.AI_TIMEOUT;
    }
    if (error.code) {
  ...
  if (axios.isAxiosError<ApiErrorBody>(error)) {
    ...
    if (/DEADLINE_EXCEEDED|timeout/i.test(body?.message || "")) {
      return errorMessageMap.AI_TIMEOUT;
    }

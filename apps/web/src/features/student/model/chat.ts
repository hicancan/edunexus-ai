...
function toPagedMeta(
  meta: Pick<PagedResult<ChatSessionVO>, "page" | "size" | "totalPages" | "totalElements">
) {
  return {
    page: meta.page,
    size: meta.size,
...
export const useChatStore = defineStore("chat", {
  ...
  actions: {
    ...
    async sendMessage(rawMessage: string): Promise<void> {
      ...
      const assistantDraft: ChatMessageVO = {
        role: "ASSISTANT",
        content: "",
        citations: [],
        createdAt: new Date().toISOString()
      };
      this.messages = [...this.messages, optimisticMessage, assistantDraft];

      try {
        let streamError = "";
        await sendChatMessageStream(this.activeSessionId, message, (event) => {
          if (event.type === "error") {
            streamError = event.message || "发送消息失败";
            return;
          }
          if (event.type !== "delta") {
            return;
          ...
          if (event.delta) {
            assistantDraft.content = `${assistantDraft.content || ""}${event.delta}`;
          }
          if (event.citations && event.citations.length > 0) {
            assistantDraft.citations = event.citations;
          }

          const next = this.messages.slice();
          next[next.length - 1] = { ...assistantDraft };
          this.messages = next;
        });
        ...
        await this.openSession(this.activeSessionId);
        await this.loadSessions({ page: this.page, size: this.size });
      } catch (error) {
        try {
          await this.openSession(this.activeSessionId);
        } catch {
          this.messages = this.messages.slice(0, Math.max(this.messages.length - 2, 0));
        }

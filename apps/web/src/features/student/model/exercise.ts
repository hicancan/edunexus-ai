...
export const useExerciseStore = defineStore("exercise", {
  getters: {
    currentAnalysis(state): ExerciseAnalysisVO | null {
      const id = state.selectedRecordId || state.latestResult?.recordId || "";
      return id ? (state.analysisByRecord[id] ?? null) : null;
    }
  },
  state: (): ExerciseState => ({
    questions: [],
  ...
  actions: {
    ...
    async loadAnalysis(recordId: string): Promise<ExerciseAnalysisVO | null> {
      ...
      if (this.analysisByRecord[recordId]) {
        return this.analysisByRecord[recordId];
      }
      this.analysisLoading = true;
      this.analysisError = "";
      try {
        const analysis = await getExerciseAnalysis(recordId);
        this.analysisByRecord = { ...this.analysisByRecord, [recordId]: analysis };
        return analysis;
      } catch (error) {

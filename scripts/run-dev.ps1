...
Start-IfPortFree "AI service" ([int]$AiPort) "Set-Location '$PSScriptRoot/../apps/ai-service'; conda run -n edunexus-ai uv sync --project . --python 3.12; conda run -n edunexus-ai uv run --project . --python 3.12 uvicorn ai_service.app:app --host $HostBind --port $AiPort"
